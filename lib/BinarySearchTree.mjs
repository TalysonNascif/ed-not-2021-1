...
export class BinarySearchTree {
    ...
    insert(val) {
        ...
        // para encontrar a pocição do novo nodo
    ...
    #insertNode(node, root) {
        ...
        // Note que, se o valor do nodo for IGUAL ao valor da raiz, nda acontece:
    ...
    posOrderTraversal(fnCallback, root = this.#root) {
        if(root !== null) {
            this.posOrderTraversal(fnCallback, root.left)   // 1º
            this.posOrderTraversal(fnCallback, root.right)  // 2º
...
let percursoPos = []
arvore.posOrderTraversal(val => percursoPos.push(val))
